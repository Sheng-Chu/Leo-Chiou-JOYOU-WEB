...
<script type="text/javascript">
	window.onload = function() {
		...
		xhr.open("GET",
				"<c:url value='/DiscountsJsonServlet.do'/>", true); //頁面預設商品
		xhr.send();
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4) {
				if (xhr.status == 200) {
					var responseData = xhr.responseText;
					displayDiscounts(responseData);
				} else {
				}
			}
		}
		
		function displayDiscounts(responseData) {
			var content = "<tr><td>代碼名稱</td><td>折扣金額(元)</td><td>折扣條件(元)</td><td>操作</td></tr>";
			var discounts = JSON.parse(responseData); 

			for (var i = 0; i<discounts.length; i++) {
				content += "<tr><td>" + discounts[i].code + "</td>";
				content += "<td>" + discounts[i].amount + "</td>";
				content += "<td>" + discounts[i].condition + "</td>";
				content += "<td><a style='margin-left:10px'data-toggle='tooltip' class='pd-setting-ed' href='${pageContext.request.contextPath}/PrepareDiscountUpdateServlet.do?";
				content += "discountid="+discounts[i].discountId+"'>";
				content += "<i class='fa fa-pencil-square-o' aria-hidden='true'></i>";
				content += "<a style='margin-left:15px' data-toggle='tooltip' class='pd-setting-ed' href='javascript: void(0)' onClick='Delete("+discounts[i].discountId+")'>";
				content += "<i class='fa fa-trash-o' aria-hidden='true'></i>";
				content += "</button></td></tr>"

			}
			document.getElementById("pageDiscounts").innerHTML = content;
	...
	function Delete(pt) {
		window.alert("折扣碼刪除成功!!!");
		var xhr = new XMLHttpRequest();
		xhr.open("GET", "<c:url value='/DeleteDiscountsServlet.do' />"
				+ "?discountId=" + pt, true);
		xhr.send();
					...
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<div class="product-status-wrap">
							...
							<div class="add-product">
								<a href="Discounts_Add.jsp">新增折扣碼</a>
							</div>

							<table id="pageDiscounts"></table>
